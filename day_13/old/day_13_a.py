class shuttle_search:
  ...
  def __init__(self, filename = 'input', thread = 1, total_thrds = 1,
               start = 0, end = 10000000):
    self.filename = filename
    self.thread = thread
    self.start = start
    self.end = end
    self.total_thrds = total_thrds
    self.test('p1')
    self.day_13('p1')
    self.test('p2')
    self.day_13('p2')
  ...
  def earliest_bus(self, earliest_time, bus_ids, bus_inds, part):
    ...
    if part == 'p1':
      ...
      for moduli in range(self.start, self.end): # 
        if moduli % bus_ids[ind_max] == target_modulus:
          target_modulus = moduli 
          break
      target_modulus += (self.thread - 1) * bus_ids[ind_max]
      counter = 0
      while True:
        target_modulus += self.total_thrds * bus_ids[ind_max]
        time_difference = [bus_id - (target_modulus % bus_id)
                           if bus_id != bus_ids[0] else 0 
                           if (target_modulus % bus_id) == 0 else 'x' 
